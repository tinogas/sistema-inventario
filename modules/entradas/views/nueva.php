...
document.getElementById('btnImportarCfdi').addEventListener('click', function () {
    ...
    fetch(APP_URL + '/api/cfdi_importar.php', { method: 'POST', body: form })
        ...
        .then(data => {
            ...
            renderTabla();
            const msg = 'CFDI importado: ' + agregadas + ' partida(s) agregada(s).'
                + (sinMatch ? ' ' + sinMatch + ' concepto(s) no encontrado(s) en el catálogo y fueron omitido(s).' : '');
            mostrarAlerta(msg, sinMatch ? 'warning' : 'success');
        })
        .catch(() => {
            const detalle = _respuestaRaw ? ' — ' + _respuestaRaw.substring(0, 120) : '';
            mostrarAlerta('Error al procesar el XML.' + detalle, 'danger');
        });
});
</script>
